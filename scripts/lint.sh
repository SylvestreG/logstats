...
if [[ -d "$inspect_path" ]]; then
  ...
  done < <(find -L "$inspect_path" -not -path "*/*build*/*" -not -path "*/CMakeFiles/*" -not -path "*/secured-container/*" -name "*.cpp" -o -name "*.[ch]" -not -path "*/*build*/*" -not -path "*/CMakeFiles/*" -not -path "*/secured-container/*" )
