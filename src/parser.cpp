...
void clf::parser::onRequest(boost::beast::http::verb request, std::filesystem::path path, httpVersion version) noexcept {
  spdlog::info("userId found:{}", boost::beast::http::to_string(request).to_string(), path.string(), clf::to_string(version));
}
...
void clf::parser::onErrorCode(boost::beast::http::status st) noexcept {
  spdlog::info("error_code:{}", boost::beast::http::obsolete_reason(st).to_string());
}
