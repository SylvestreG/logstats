...
cfl::Config::Config(std::filesystem::path file) {
  ...
  try {
    ...
    auto getParam = [&jf](char const *paramName, auto& valueToUpdate, int minValue, int maxValue) {
      if (jf[paramName].is_number_integer()) {
        ...
        if constexpr (std::is_same_v<std::chrono::milliseconds &, decltype(valueToUpdate)>) {
    ...
    getParam("batchMaxSizeNumber", _batchMaxSizeNumber, 0, std::numeric_limits<uint16_t>::max());
    getParam("batchMaxTimeMs", _batchMaxTime, 0, std::numeric_limits<uint16_t>::max());
    getParam("consumerThreadsNumber", _consumerThreadsNumber, 0, std::numeric_limits<uint8_t>::max());
    getParam("refreshTimeMs", _refreshTimeMs, 0, std::numeric_limits<uint16_t>::max());
    getParam("alertThresholdNumber", _alertThresholdNumber, 0, std::numeric_limits<uint16_t>::max());
