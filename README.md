...
The clfGenerator generator will generate an *N* log lines
every *T* interval of times into the output file . The behaviour of  this program is defined
by its config file.
...
we could remove this tick and set a timer to then end of the alert period. (updating it if a
new lines come before the trig)
